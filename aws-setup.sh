...
role_response=$(aws iam get-role --role-name "$ROLE_NAME" || aws iam create-role --role-name "$ROLE_NAME" --assume-role-policy-document file://$(pwd)/config/aws-lambda-role-policy.json)
...
#--environment "Variables={AWS_SNS_ARN=$sns_topic_arn}"
